...
| 展开面板底部 | 左下角 `当前时段：高峰时段/空闲时段`；右下角右对齐 `余额：9.01 CNY`（样式同面板头部 `~244K` 数字） |
...
展开面板是 React 管理的子树，向其注入真实 DOM 节点会导致 React 重渲染崩溃（曾出现整页卡死 + `fetchError`）。因此：

- 面板底部状态行用 `::before` / `::after` 绝对定位伪元素渲染（只写 `data-*` 属性，React 无感）；
- 悬浮气泡是圆环根容器的直接子节点（Tooltip 用 Fragment 渲染），改写其 `textContent` 属于文本节点更新，安全。
